...
def export_violations_excel(violations, program_name: str = "Analysis") -> bytes:
    ...
    ws2["A1"] = "CoreShift Analysis Summary"
...
def export_pdf(title: str, content: str, program_name: str = "") -> bytes:
    ...
    story.append(Paragraph("CoreShift", title_style))
...
def _add_cv_header(doc, title: str, program_name: str) -> None:
    ...
    run = p.add_run("CoreShift — Enterprise ABAP Intelligence Platform")
    run.bold = True
    run.font.size = Pt(16)
...
def _add_footer(doc) -> None:
    ...
    doc.add_paragraph("─" * 80)
    p = doc.add_paragraph("Generated by CoreShift — Enterprise ABAP Intelligence Platform")
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
